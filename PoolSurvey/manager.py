...
try:
    ...
    from simul import GPIO
    simu = True

class Manager(Debug, threading.Thread):
    class Fake: pass
    ACCOUNT_SID = "AC7a452e3f660c7759a73f2b5a4026790c"
    ACCOUNT_AUTH = "8391d2739f07a344d4b246aa853e03ef"
    ACCOUNT_PHONE_NUMBER = "+33756796236"
    DEFAULT_CRITICAL = 1
    DEFAULT_IMPORTANT = 2
    ...
    def __init__(self, stdout=None):
        if stdout:
            ...
            self.__simu = False # Use thread
        else:
            self.__simu = True
        Debug.__init__(self, [Debug.ERROR, Debug.DEBUG][self.__simu])
...
#            self.lcd.write(1, 0, "Etat=%s" % ["Arret", "Marche"][self.state.pump])
    ...
    def start(self):
        ...
        if self.__simu:
            delayTick = self.REFRESH_TICK / 10
            while self.__simu:
                self.TAG(self.DEBUG, "%ds Tick" % delayTick)
                self.__refresh()
    ...
    def alert(self, message):
        ...
        phoneNumbers = ("0643648483")
        for number in phoneNumbers:
            self.sms.messages.create(from_=self.ACCOUNT_PHONE_NUMBER, to="+33"+number[-9:], body=message)

if __name__ == '__main__':
    manager = Manager([sys.stdout, None][simu])
##    for temp in range(115, 325, 3):
    for temp in range(290,380, 3):
        manager.program.auto = [0]
        tmp = float(temp) / 10
        manager._Manager__computePumpScheduling(tmp)
        print 20 * '#', ' TEMP = %.1f ' % tmp , 20 * '#'
        for item in manager.program.auto[1:]:
            print item
    #try:
        #manager.start()
